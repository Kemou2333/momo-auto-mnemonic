...
3. 主 Claude 读 `CLAUDE.md` 了解流程。**主 agent 不读 `MNEMONIC_RULES.md`**——
   规则文件较长，由子 agent 各自读，避免把主上下文占满。
4. 主 Claude 跑 `python3 run_mnemonics.py --fetch` 拉今日剩余+明日待处理词，
   按 CLAUDE.md 里的"词数 → 子 agent 数"对照表决定派几个子 agent。
5. **串行**派发 Sonnet 子 agent（一次只派一个）。每个子 agent 自己 view
   `MNEMONIC_RULES.md`，生成助记后用 Edit 工具**直接**写入
   `run_mnemonics.py` 的 `ALL_NOTES` 列表（用 `]\n\nALL_PHRASES = [` 作为
   定位锚点，串行派发保证锚点不被并发改坏）。子 agent 只回报 voc_id 列表，
   **不向主 agent 返回助记内容**。
6. 全部子 agent 完成后，主 Claude 跑 `python3 run_mnemonics.py` 提交。
...
### 为什么主 agent 派子 agent 写助记，而不是自己写

早期版本是主 Claude 自己读规则 + 生成助记 + Edit 写入脚本。70 词时实测一次跑掉
将近一半 5 小时额度——主上下文同时持有 10K 规则文件 + 14K 元组生成 + 14K Edit 工具
调用 ≈ 38K，且因为这些内容是"答案"不是"中间产物"，主 agent 无法卸载。

改成"主 agent 协调 + 子 agent 干活"架构后：

- 主 agent 不读 `MNEMONIC_RULES.md`
- 主 agent 不接收子 agent 生成的助记内容（子 agent 直接 Edit 脚本）
- 主 agent 只持有：CLAUDE.md + `--fetch` 输出的词列表 + 子 agent 完成回报
- 子 agent 的上下文用完即丢，不污染主上下文

净效果：主上下文负载从 ~38K 降到 ~3K，总 token 也比"子 agent 返回文本再让主 agent
Edit"省 ~13K（避免了 14K 元组在子→主→Edit 之间走两遍）。

### 为什么子 agent 串行而不是并行

子 agent 直接 Edit 同一个 `run_mnemonics.py` 文件，并行会撞 Edit 锚点（第一个子
agent 一改文件，第二个的 old_string 就匹配不到了）。

串行的代价是 wall time × N，但 Routine 一天跑一次不在乎几分钟差异。
真要并行需要让每个子 agent 写各自的临时文件再让脚本合并，工程量翻倍不值。

### 为什么不是一个子 agent 处理全部词

理论上一个子 agent 一锅端最省 token（规则文件只读一次）。但实测有两个问题：

1. **输出过长质量下滑**：70 词 = ~14K 输出，Sonnet 后半段倾向于敷衍
2. **失败不可分**：中途崩了整批丢失

折中：按词数派 N 个子 agent，每个处理 ~25 词。CLAUDE.md 给出阈值表：
≤40 一个、41~60 两个、61~90 三个，之后每 30 词加一个。
